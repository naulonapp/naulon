...
the thirteen), and `modularChainName: "arc"`, so Circle's Modular Wallets accept the chain, measured
...
now carries a `rules[]` array of pattern, read leg, citation leg and the multiplier that
...
enforce, because a second copy of a sanitizer is how one of them stops sanitizing.
...
- **The plugin's payment challenge points at the licence.** A 402 carries
  ...
  has been published, so nothing is advertised that is not served.
...
- **`@naulon/wayfarer` honours all of it.** `spendGate`, the one evaluator both the research run
  and the MCP pay tool call use, refuses a read the publisher prohibits, refuses an **undischarged**
...
- **`gateScope.includeExtensions`**: a whole-site publisher can opt a file extension
  ...
  toll a sitemap, because tolling discovery would starve the catalog agents buy from.
...
- **`@naulon/shared`: `readAllPaged()` and `PAGE_ROWS`.** One paging primitive for PostgREST
  ...
  prefix of the truth rather than an error; this walks the pages instead. Additive, with no existing
  export changed signature.

## v0.7.3

Ships `@naulon/shared` 0.4.0 · `@naulon/enforce` 0.4.0 · `@naulon/sdk` 0.3.1 ·
`@naulon/wayfarer` 0.3.1 · `@naulon/wayfarer-mcp` 0.4.2. The two minors are additive, with new
exports and no signature changed, but a caret range on a zero-major is minor-tight, so anything
...
- **`@naulon/shared`: `ForgoneLeg` and `AttributedEvent.forgone`.** Every leg the quote required
  ...
  never a sum, because a total cannot say whose money it was, and an operator leg and a co-author leg are
...
WordPress plugin only, the same shape as v0.7.2 above: no npm package moved, and the
plugin's own changelog carries the detail.

## v0.7.0

Ships `@naulon/sdk` 0.3.0 · `@naulon/shared` 0.3.1 · `@naulon/enforce` 0.3.1 ·
`@naulon/wayfarer-mcp` 0.4.1. The middle two carry no behaviour change, since their
...
- **The Docker image could not boot at all.** `tsx` was a root devDependency while the image
  ...
  dependency, because nothing in this repo is compiled before it runs, which makes it a runtime need,
  not a dev one.
...
- **`@naulon/sdk`: the settlement receiver is gone; receive a webhook instead.**
  ...
  type rather than settlements only, and `idempotency` is **required**, because delivery is
...
- **Adapters can no longer key an article.** `discover` returns `ArticleCandidate[]`;
  ...
  could not key (`CrawlResult.unkeyable`, where a big number usually means the wrong
...
- **`@naulon/wayfarer`: Arc-mainnet funding calls silently stopped opting into the
  ...
  half, meaning deposit, withdraw and balances, while settle kept working.
...
- **`@naulon/shared` and `@naulon/enforce`: Web Bot Auth signed the scheme the socket

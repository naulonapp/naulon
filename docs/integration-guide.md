...
> side, and naulon calls you. Works the same whether you run your own gate or sit behind
...
The core (`@naulon/sdk`) depends only on `zod`, so it's framework-free. The
...
only and is not durable, so do not ship it.
...
| `/credits` reachability | may be internal or localhost (gate is co-located) | must be public internet, so set a bearer `CREDITS_API_TOKEN` |

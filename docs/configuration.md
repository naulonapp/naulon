...
| `TRUST_PROXY` | `false` | Read the client IP from `X-Forwarded-For`. Turn it **on** whenever anything sits in front of the gate — with it off, every request keys to the proxy's address, the whole deployment shares one bucket, and a single caller can 429 everyone. The trail is read from the right, so a client-forged entry is ignored; the proxy still has to be one you control. |

...
- **`404` means free.** A 404 is the deliberate "don't gate this slug" signal, so the
...
| Rule | Detail |
|---|---|
| Leaf **XOR** composite | Each contributor has **exactly one** of `wallet` (leaf) or `members` (composite, ≥1), never both and never neither. |
...
| Strict | The object is `.strict()`, so any unknown key is rejected. |
...
The resolver is your policy. Everything site-specific, meaning which slugs are free, how an
author maps to a wallet, and membership rules, lives in *your* `CreditsResolver`; the
...
- **Secure the endpoint.** The gate validates that a response is *well-formed*, but
  ...
  payment, so keep its auth tight and serve it over TLS.
...
A complete, runnable consumer (this endpoint + the signed settlement webhook,
against a static fixture) lives in

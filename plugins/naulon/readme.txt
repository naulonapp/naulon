...
* Titles, excerpts, your sitemap and your feeds stay free, because agents need to find you.
...
3. Click **Verify this site**. The plugin proves you own the domain by serving a challenge file and a meta tag, with no DNS changes needed.
...
Their posts still credit them, with no destination attached. If the service this site connects to holds a payout wallet for that author, their share goes there. If nothing does, the post reads free, because nothing is charged when there is nobody to pay.
...
If you do want everything removed, **naulon → Diagnostics** has a box for it, next to a count of exactly what it would destroy and a button to export a copy first. That order is deliberate, because WordPress removes a plugin's data before it removes the plugin's files, and it cannot be undone.
...
4. Earnings: what has been paid, per author, with settled and authorized shown separately.
...
* A report that cannot be delivered, through a timeout or your site being offline for a moment, is kept and sent with the next one, rather than lost.
...
* Diagnostics now tells you if the plugin's own folder is not writable by your web server, which is the reason an update reports that files could not be copied, or a delete reports that the plugin could not be fully removed. It names the folder and what to fix.
...
* A spent connect key reads as finished instead of failing, because the key drops its domain-management scope once the domain verifies.
...
= 0.5.0 =
Your site now states its terms where a crawler looks for them: /license.xml, a line in robots.txt, and a link in every page head. Built from the price you already set, with nothing to configure.
